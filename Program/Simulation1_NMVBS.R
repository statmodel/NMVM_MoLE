...
q=3
p=3
beta0=matrix(c(0.2,2,4,0.5,-1.5,-1),ncol=2)
sigma20 = c(1, 2)
lambda0 = c(3, -2)
alpha0 = matrix(c(-3,-1,2),ncol=1,byrow=TRUE)
theta0 = cbind(c(2,4), c(1.5,3))
for (m in 1:length(nsize)) {
    ...
    while(l <= (nrun - 1)){
      l = l + 1
      x=cbind(rep(1,n),runif(n,0,1),runif(n,-1,1))
      r=cbind(rep(1,n),runif(n,-2,2),runif(n,0,1))
      out.N = r.mix.reg.NMV(n,x,beta0, sigma20, lambda0, theta0, alpha0,r, family = "NMVBS")
      Class = out.N$class
      y = out.N$y
      data <- data.frame(Y = y, Class = out.N$class)
      Out.NMVBS = mix.reg.NMVBS.EM(y, x,beta=NULL, sigma2 = NULL,
      ...
      result1[,l] = t(as.matrix(c(as.vector(Out.NMVBS$sigma2), as.vector(Out.NMVBS$lambda), as.vector(Out.NMVBS$alpha),
                                  as.vector(Out.NMVBS$beta), as.vector(Out.NMVBS$alpha2))))
    ...
    write.csv(result1, paste("Sim1/NMVBS_result1_",nsize[m],per[k],".csv"), append = TRUE,row.names = F,col.names = F)
    write.csv(result2, paste("Sim1/NMVBS_result2_",nsize[m],per[k],".csv"), append = TRUE,row.names = F,col.names = F)
    write.csv(result3, paste("Sim1/NMVBS_result3_",nsize[m],per[k],".csv"), append = TRUE,row.names = F,col.names = F)
    print(n)
}
proc.time()-ptm
end.time <- Sys.time()
