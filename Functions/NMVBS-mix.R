...
mix.reg.NMVBS.EM <- function(y, x, beta=NULL, sigma2 = NULL,
                                    ...
                                    error.est = F, per = 1, print = T, fix.sigma = F)
{
  ...
  fGH = function(x, tau, sigma2, kappa, chi, psi, log.val = F){
    chii = chi + (x)^2 / sigma2
    ...
    out3 = (x) * tau / sigma2 - 0.5 * log(sigma2 * 2 * pi)
    F1 = out1 + out2 + out3
    ifelse(log.val == T, return(F1), return(exp(F1)))
  }
  
  f.NMVBS = function(x, sigma2, lambda, alpha2,log.val = F) {
    0.5 * (fGH(x, lambda, sigma2, 0.5, alpha2^(-2), alpha2^(-2), log.val = F)+
             fGH(x, lambda, sigma2, -0.5, alpha2^(-2), alpha2^(-2), log.val = F))
  }
  
  d.NMVBS.mix = function(y, x, beta, sigma2, lambda, r, alpha, alpha2,class, log.val = F)
  {
    g = length(sigma2)
    PDF = 0
    for(j in 1:g){
      ...
      for(i in 1:length(Cent)){
        ...
        PDF = PDF + pI * f.NMVBS(Cent[i], sigma2[j], lambda[j], alpha2[j], log.val = F)
      }
    }
    Out = sum(PDF)
  ...
  if(isTRUE(print)){
    ...
    cat('Finite mixture of NMVBS-based reg model','\n')
  }
  n = length(y)
  ...
  ##----------------- Expectations -----------------------##
  ##------------------------------------------------------##
  EUY.NMVBS = function(y,x,beta, sigma2, lambda, alpha2)
  {
    R = function(cc, k, a) BesselK(cc, k+a, log.val = T) - BesselK(cc, k, log.val = T)
    
    psii = alpha2^(-2) + (lambda)^2 / sigma2
    erii=x%*%beta
    pp = 1/ (1 + exp(fGH((y-erii), lambda, sigma2, -0.5, alpha2^(-2), alpha2^(-2), log.val = T) -
                       fGH((y-erii), lambda, sigma2, 0.5, alpha2^(-2), alpha2^(-2), log.val = T)))
    chiij = alpha2^(-2) + (y-erii)^2 / sigma2
  ...
  start.time = Sys.time()
  lk = lk.old = sum(d.NMVBS.mix(y, x, beta, sigma2, lambda, r, alpha, alpha2, class, log.val = T))
  ...
  repeat
  {
    ...
    for(j in 1:g){
      for(i in which(class==j)){
        ...
        tal[i, j] =pI[j] * f.NMVBS(y[i]-e, sigma2[j], lambda[j], alpha2[j], log.val = F)
      }
    }
    ...
    for (j in 1:g){
      u=w=rep(0,0)
      for(i in which(class==j)){
        Ewt = EUY.NMVBS(y[i], x[i,],beta[,j],sigma2[j], lambda[j],alpha2[,j])
      ...
      alpha2[j] =sqrt(sum(sij )/sum(tal[, j]))
    }
    
    if(fix.sigma) sigma2 = rep(SS/(n), g)
    
    lk.new = sum(d.NMVBS.mix(y, x, beta, sigma2, lambda, r, alpha, alpha2,class,log.val = T))
