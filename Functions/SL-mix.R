...
mix.reg.SLap.EM <- function(y, x, beta=NULL, sigma2 = NULL,
                                   lambda = NULL,  r, alpha=NULL, g = NULL, 
                                   Class = NULL, p=NULL,error = 0.00001,
                                   iter.max = 100, Stp.rule = 
                                     c("Log.like", "Atiken"), 
                                   error.est = F, per = 1, print = T,
                                   fix.sigma = F)
{
  f.SL = function(y, sigma2, lambda){
    sigma = sqrt(sigma2)
    tau = sqrt( 1 + (lambda/sigma)^2 )
    Out = exp( (-tau * abs(y ) / sigma) + lambda * (y) / sigma^2) / 
      (2 * tau * sigma)
    return(Out)
  }
  
  d.SLap.mix <- function(y, x, beta, sigma2, lambda, r, alpha, class, log.val = F)
  {
    g = length(sigma2)
    ...
    for(j in 1:g){
      ...
      for(i in 1:length(Cent)){
        ...
        PDF = PDF+ pI * f.SL(Cent[i], sigma2[g], lambda[j])
      }
    }
    Out = sum(PDF)
  ...
  if(isTRUE(print)){
    ...
    cat('Finite mixture of Skew-Laplace-based reg model','\n')
  }
  n = length(y)
  ...
  EUY.SLap = function(y,x,beta, sigma2, lambda,r,alpha)
  {
  ...
  start.time = Sys.time()
  lk = lk.old = sum(d.SLap.mix(y, x, beta, sigma2, lambda, r, alpha ,class, log.val = T))
  ...
  repeat
  {
    ...
    for(j in 1:g){
      for(i in which(class==j)){
        ...
        tal[i, j] =pI[j] * f.SL(y[i]-e, sigma2[j], lambda[j])
      }
    }
    ...
    for (j in 1:g){
      ...
      for(i in which(class==j)){
        ### E-step: 
        Ewt = EUY.SLap(y[i], x[i,],beta[,j],sigma2[j], lambda[j], r[i,],alpha)
    ...
    lk.new = sum(d.SLap.mix(y, x, beta, sigma2, lambda, r, alpha, class, log.val = T))
  ...
  m = g * (nrow(beta) + 2) + nrow(alpha)*(g - 1)
  ...
  if(!is.null(Class)){
    ...
    if(error.est == T){
      SS = 0
      for(i in 1:n){
        sp = ssigma = slambda = numeric(g); salpha = matrix(0, q, g)
        for (j in 1:g)
        {
          EUTT = EUY.SLap(y[,i], sigma2[class[i]], lambda[class[i]], alpha[class[i]])
          w = Ewt$w; u = Ewt$t
          h=matrix(0,(n-q),N)
          for(i in 1:N){
            if(class[i]==j){
              for(t in 1:(n-q)){
                h[t,i]<-(1-sum(alpha[,j]))+sum(alpha[,j]*y[(t+q-1):(t),i]^2)
              }
            }
          }
          Cent=y[(q+1):n,class==j]/sqrt(h[,class==j])
          
          sp[j] = (tal[i, j]/pI[j]) - (tal[i, g]/pI[g])
          slambda[j] = tal[i, j] * (Cent - w*lambda[j]) / sigma2[j]
          ssigma[j] = - tal[i, j] * (sigma2[j] - u * Cent^2 + 
      ...
      obj.out = list( time = time.taken, group = Group, m = m,
                      sigma2 = sigma2, lambda = lambda, beta = beta,alpha=alpha, 
