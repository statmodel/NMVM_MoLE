...
mix.reg.norm.EM <- function(y, x, beta=NULL, sigma2 = NULL, 
                                   g = NULL, r, alpha=NULL, Class = NULL,
                                   error = 0.00001,p=NULL, iter.max = 100, 
                                   Stp.rule = c("Log.like", "Atiken"), 
                                   error.est = F, per = 1, print = T, fix.sigma = F)
{
  if(isTRUE(print)){
    cat(paste(rep("-", 70), sep = "", collapse = ""), "\n")
    cat('Finite mixture of normally-based reg model','\n')
  }
  n = length(y)
  
  if(is.null(sigma2) || is.null(alpha) || is.null(beta) ){
  ...
  d.Norm.mix <- function(y, x, beta, sigma2, r, alpha,class, log = F)
  {
    g = length(sigma2)
    sigma = sqrt(sigma2)
    PDF = 0
    for(j in 1:g){
      ...
      for(i in 1:length(Cent)){
        ...
        PDF = PDF + pI* dnorm(Cent[i],0, sigma[j], log = F)
      }
    }
    Out = sum(PDF)
  ...
  start.time = Sys.time()
  lk = lk.old = sum(d.Norm.mix(y, x, beta, sigma2, r, alpha, class, log = T))
  ...
  repeat
  {
    ...
    for(j in 1:g){
      for(i in which(class==j)){
        ...
        tal[i, j] = pI[j] * (dnorm(y[i]-e, 0,sqrt(sigma2[j]), log= F))
      }
    }
    ...
    lk.new = sum(d.Norm.mix(y, x, beta, sigma2, r, alpha, class, log = T))
  ...
  m <- g * (nrow(beta) + 1) + nrow(alpha)*(g-1) 
  if(fix.sigma) m = m - g + 1
  
  aic = -2 * lk + 2 * m
  bic = -2 * lk + log(n-p) * m
  edc = -2 * lk + 0.2 * sqrt(n) * m
  aic_c = -2 * lk + 2 * n * m / (n -p- m - 1)
  abic  = -2 * lk + m * log((n-p + 2) / 24)
  ...
  if(!is.null(Class)){
    ...
    obj.out = list( time = time.taken, group = Group, m = m,
                    sigma2 = sigma2, beta = beta, alpha=alpha, loglike = lk, aic = aic,
