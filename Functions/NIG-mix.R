...
mix.reg.nig.EM = function(y, x, beta=NULL, sigma2 = NULL,
                                 ...
                                 error.est = F, per = 1, print = T, fix.sigma = F)
{
  ...
  f.nig = function(y, sigma2, lambda, nu, log.val = F){
    ob = ghyp :: NIG(chi = 1, psi = nu, mu = 0, 
                     sigma = sqrt(sigma2), gamma = lambda)
    Val= ghyp :: dghyp(y, object = ob, logvalue = log.val)
    Val
  }
  
  d.nig.mix = function(y, x, beta, sigma2, lambda, nu, r, alpha,class,log = F)
  {
    g = length(sigma2)
    PDF = 0
    for(j in 1:g){
      ...
      for(i in 1:length(Cent)){
        ...
        PDF = PDF + pI * f.nig(Cent[i], sigma2[j], lambda[j], nu[j], log.val = F)
      }
    }
    Out = sum(PDF)
  ...
  if(isTRUE(print)){
    ...
    cat('Finite mixture of NIG-based reg model','\n')
  }
  n = length(y)
  ...
  start.time = Sys.time()
  lk = lk.old = sum(d.nig.mix(y, x, beta, sigma2, lambda, nu, r, alpha,class,log = T))
  criterio = 1; count = 0
  if(isTRUE(print)){
    cat(paste(rep("-", 70), sep = "", collapse = ""), "\n")
  ...
  repeat
  {
    ...
    for(j in 1:g){
      for(i in which(class==j)){
        ...
        tal[i, j] =pI[j] * f.nig(y[i]-e, sigma2[j], lambda[j], nu[j], log.val = F)
      }
    }
    ...
    for (j in 1:g)
    {
      ...
      for(i in which(class==j)){
        ### E-step: 
        Ewt = EUY.nig(y[i], x[i,],beta[,j],sigma2[j], lambda[j], nu[j])
      ...
      nu[j] = sum(tal[, j])/sum(w)
    }
    if(fix.sigma) sigma2 = rep(SS/(n), g)
    
    
    lk.new = sum(d.nig.mix(y,x,beta,sigma2,lambda, nu, r, alpha,class, log = T))
  ...
  if(!is.null(Class)){
    ...
    obj.out = 
      list( time = time.taken, group = Group, m = m,
            sigma2 = sigma2, lambda = lambda, nu=nu, 
            beta = beta, alpha=alpha, loglike = lk, aic = aic, bic = bic, edc = edc, 
